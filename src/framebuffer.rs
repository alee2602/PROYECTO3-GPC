    ...
    pub fn line(&mut self, x0: usize, y0: usize, x1: usize, y1: usize) {
        ...
        loop {
            ...
            if x0 >= 0 && x0 < self.width as isize && y0 >= 0 && y0 < self.height as isize {
                self.point(x0 as usize, y0 as usize, 0.0); // Profundidad 0.0 para líneas
            }

            // Salir si hemos llegado al punto final
            if x0 == x1 && y0 == y1 {
                break;
            }

            // Actualizar el error y mover los puntos según el algoritmo de Bresenham
            let e2 = 2 * err;
            if e2 > -dy {
                err -= dy;
                x0 += sx;
